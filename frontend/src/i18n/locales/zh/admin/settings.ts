export default {
    settings: {
      ...
      openaiExperimentalScheduler: {
        ...
        candidateIndexTitle: '智能候选路由引擎',
        candidateIndexDescription: '开启后 OpenAI/Grok 选号从全量快照扫描切换为 Redis 小候选池读取，账号失效、限流或超时会被快速移出候选池。大号池、高并发或失败账号较多时，可明显降低 CPU、Redis 读量和 P95 延迟；小号池、低并发且账号稳定时可保持关闭。',
        candidateConfigTitle: '智能候选路由配置',
        candidateConfigDescription: '只配置会影响线上请求路径的运行时参数；保存后开关状态仍以右侧状态标签为准。',
        candidateFetchLimitLabel: '候选池读取量',
        candidateFetchLimitDescription: '每次请求最多从 Redis 候选集合读取的账号数。值越大命中率越高，但单请求过滤和 MGET 成本也越高。',
        candidateFetchLimitRecommendation: '推荐：少于 30 个账号填 16-32；30-200 个填 64；200-1000 个填 96-128；超过 1000 个填 128-256。最低 8。',
        candidateReadyWaitLabel: 'ready 传播等待（ms）',
        candidateReadyWaitDescription: '等待刚写入的 ready 标记在 Redis/多实例间可见，用于覆盖极短传播窗口。',
        candidateReadyWaitRecommendation: '推荐：单机或低延迟 Redis 填 100-200；跨机房或 Redis 延迟偏高填 300-500。',
        candidateBuildWaitLabel: '缺索引构建等待（ms）',
        candidateBuildWaitDescription: '启动恢复或 bucket 缺失时，新请求最多等待定向索引构建完成的时间；超时会返回 cache not ready。',
        candidateBuildWaitRecommendation: '推荐：小号池填 3000-5000；中号池填 5000-10000；大号池填 10000-20000。',
        candidateIndexDisabled: '未启用',
        candidateIndexBuilding: '切换中',
        candidateIndexActive: '已启用',
        candidateIndexFailed: '失败',
        topKLabel: 'TopK',
        priorityWeight: '优先级',
        loadWeight: '负载',
        queueWeight: '排队',
