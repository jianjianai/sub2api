  ...
  admin: {
    ...
    accounts: {
      ...
      schedulerScore: {
        ...
        stickyShort: 'Sticky',
        ungrouped: 'Ungrouped',
        pendingTooltip: 'The scheduler score read model is not ready yet; it will be generated after the scheduler snapshot rebuilds.',
        hint: 'Displayed as "group / base score / sticky bonus". The base score comes from the real scheduler bucket candidate snapshot and includes priority, load, queue depth, reset window, quota headroom, and related snapshot factors. Runtime error rate and first-token latency are not part of this read model yet. The sticky bonus applies only when sticky weighting is enabled for previous_response_id or session_hash. Higher scores are preferred.'
      },
    ...
    settings: {
      ...
      openaiExperimentalScheduler: {
        ...
        candidateIndexTitle: 'OpenAI Candidate Index Scheduler',
        candidateIndexDescription: 'When enabled, scheduler snapshots build a candidate index and OpenAI account selection prefers paged index scans. Missing indexes or scan limits still fall back to the legacy path.',
        candidateIndexPerformance: 'The gain is mostly in the selection phase. With hundreds to thousands of accounts, heavy group/model filtering, or amplified Redis load reads, it typically cuts about 70%-90% of candidate scanning and batch load reads. Below 100 accounts the gain is usually small; use latency monitoring as the final signal.',
        candidateIndexSettingsTitle: 'Candidate index parameters',
        candidateIndexRecommendation: 'Recommended for 200+ schedulable OpenAI accounts, rising selector latency, or visible Redis read pressure. Small low-concurrency pools can usually leave it disabled.',
        candidateIndexPageSizeLabel: 'Candidate page size',
        candidateIndexPageSizeHint: 'Default 256. Use 256 for 200-1000 accounts, 512 for 1000+ accounts or many filters, and 128 when hit rate is high and each read should be smaller.',
        candidateIndexMaxScanLabel: 'Max scanned candidates',
        candidateIndexMaxScanHint: 'Default 2000. Use 500-1000 below 500 accounts, 2000 for 500-3000 accounts, and 5000 for 3000+ accounts or heavy model/group filtering.',
