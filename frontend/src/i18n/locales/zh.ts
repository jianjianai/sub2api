  ...
  admin: {
    ...
    settings: {
      ...
      openaiExperimentalScheduler: {
        ...
        candidateIndexTitle: 'OpenAI 候选索引调度引擎',
        candidateIndexDescription: '开启后调度快照会构建候选索引，OpenAI 账号选择优先走索引分页扫描；索引缺失或扫描超限时仍自动回退旧路径。',
        candidateIndexPerformance: '性能收益主要体现在选号阶段：账号池达到数百到数千规模、分组/模型过滤较多或 Redis 负载读取放大时，通常可减少约 70%-90% 的候选扫描与批量负载读取；100 个以内账号收益通常不明显，具体以监控延迟为准。',
        candidateIndexSettingsTitle: '候选索引参数',
        candidateIndexRecommendation: '推荐：200 个以上 OpenAI 可调度账号、选号延迟升高或 Redis 读压力明显时开启；几十个账号或低并发环境可保持关闭。',
        candidateIndexPageSizeLabel: '候选页大小',
        candidateIndexPageSizeHint: '默认 256。200-1000 个账号建议 256；1000+ 或过滤条件多建议 512；命中率高且追求更低单次读取可用 128。',
        candidateIndexMaxScanLabel: '最大扫描候选数',
        candidateIndexMaxScanHint: '默认 2000。500 个以内账号建议 500-1000；500-3000 个账号建议 2000；3000+ 或模型/分组过滤较重建议 5000。',
        weightsTitle: '调度权值覆盖',
