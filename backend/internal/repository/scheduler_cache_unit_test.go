...
func TestSchedulerCacheUpdateCandidateScoresClearsUnencodablePayload(t *testing.T) {
	ctx := context.Background()
	cache := newSchedulerCacheUnit(t)
	...
	require.NoError(t, cache.SetAccount(ctx, &account))
	require.NoError(t, cache.rdb.SAdd(ctx, schedulerAccountBucketsKey("114"), bucket.String()).Err())

	invalidTime := time.Date(10000, time.January, 1, 0, 0, 0, 0, time.UTC)
	require.NoError(t, cache.UpdateCandidateScores(ctx, map[int64]time.Time{account.ID: invalidTime}))
